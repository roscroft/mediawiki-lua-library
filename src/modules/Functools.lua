...
---@param a number First number
---@param b number Second number
---@return number Productfunction func.ops.mul(a, b)    return a * bend---Subtraction operation---@param a number First number---@param b number Second number---@return number Differencefunction func.ops.sub(a, b)    return a - bend---Division operation---@param a number First number---@param b number Second number---@return number Quotientfunction func.ops.div(a, b)    if b == 0 then        error("Division by zero", 2)    end    return a / bend---Modulo operation---@param a number First number---@param b number Second number---@return number Remainderfunction func.ops.mod(a, b)    return a % bend---Power operation---@param a number Base---@param b number Exponent---@return number Resultfunction func.ops.pow(a, b)    return a ^ bend---Equality comparison---@param a any First value---@param b any Second value---@return boolean Equalfunction func.ops.eq(a, b)    return a == bend---Less than comparison---@param a any First value---@param b any Second value---@return boolean Less thanfunction func.ops.lt(a, b)    return a < bend---Greater than comparison---@param a any First value---@param b any Second value---@return boolean Greater thanfunction func.ops.gt(a, b)    return a > bend-- ======================-- MEMOIZATION & RECURSION - Performance optimization and recursive utilities-- ======================---Memoize a function for single argument---@generic A, B---@param f fun(x: A): B Function to memoize---@return fun(x: A): B Memoized functionfunction func.memoize(f)    local cache = {}    return function(x)        local key = tostring(x)        if cache[key] == nil then            cache[key] = f(x)        end        return cache[key]    endend---Add trampolining for stack safety in recursive calls with functional improvements---@generic A, R---@param f fun(...: A): R|function Function that may return another function for trampolining---@return fun(...: A): R Function with trampolining appliedfunction func.trampoline(f)    return function(...)        -- Use a functional approach to repeatedly apply until we get a non-function result        local result = f(...)        local continue = function(r) return type(r) == "function" end        while continue(result) do            result = result()        end        return result    endend---Functional until combinator - repeatedly apply function until predicate is true---@generic T---@param predicate fun(x: T): boolean Predicate to test---@param transform fun(x: T): T Function to apply---@return fun(x: T): T Function that applies transform until predicate is truefunction func.until_fn(predicate, transform)    return function(initial)        local current = initial        while not predicate(current) do            current = transform(current)        end        return current    endend---Functional while combinator - repeatedly apply function while predicate is true---@generic T---@param predicate fun(x: T): boolean Predicate to test---@param transform fun(x: T): T Function to apply---@return fun(x: T): T Function that applies transform while predicate is truefunction func.while_fn(predicate, transform)    return func.until_fn(func.complement(predicate), transform)end---Fixed-point combinator (Y combinator approximation)---@generic T, R---@param f fun(rec: fun(x: T): R): fun(x: T): R Function that takes its own recursive call---@return fun(x: T): R Recursive functionfunction func.fix(f)    checkType('Module:Functools.fix', 1, f, 'function')    local function rec(...)        return f(rec)(...)    end    return recend-- ======================-- UTILITY FUNCTIONS & PREDICATES - General purpose utilities and type checks-- ======================---Check if value is empty (nil, empty string, or empty table)---@param value any Value to check---@return boolean True if emptyfunction func.is_empty(value)    if value == nil or value == "" then        return true    end    if type(value) == "table" then        return next(value) == nil    end    return falseend---Deep clone a table---@generic T---@param obj T Table to clone---@return T Cloned tablefunction func.deep_clone(obj)    return func.fix(function(self)        return function(x)            if type(x) ~= "table" then return x end            local result = {}            for k, v in pairs(x) do                result[self(k)] = self(v)            end            return setmetatable(result, getmetatable(x))        end    end)(obj)end---Complement a predicate function---@generic T---@param predicate fun(x: T): boolean Predicate to complement---@return fun(x: T): boolean Complemented predicatefunction func.complement(predicate)    return function(x)        return not predicate(x)    endend---Create a predicate that checks if value is in list---@generic T---@param list T[] List of values to check against---@return fun(x: T): boolean Predicate functionfunction func.contains_in(list)    local lookup = {}    for _, v in ipairs(list) do        lookup[v] = true    end    return function(x)        return lookup[x] == true    endend---Check if input has specific type---@param type_name string Type name to check---@return fun(value: any): boolean Predicate that checks if value is of specified typefunction func.is_type(type_name)    return function(value)        return type(value) == type_name    endend-- Create common type predicatesfunc.is_function = func.is_type("function")func.is_table = func.is_type("table")func.is_string = func.is_type("string")func.is_number = func.is_type("number")func.is_boolean = func.is_type("boolean")-- ======================-- ADVANCED UTILITIES - Lenses and Transducers-- ======================---Create a lens for functional data access and modification---@generic S, A---@param getter fun(s: S): A Function to get value from structure---@param setter fun(a: A): fun(s: S): S Function to set value in structure---@return table<string, function> Lens object with get, set, and modify operationsfunction func.lens(getter, setter)    return {        get = getter,        set = setter,        modify = function(f)            return function(s)                return setter(f(getter(s)))(s)            end        end    }end---Focus on a specific key in a table (lens for table access)---@generic K, V---@param key K Key to focus on---@return table<string, function> Lens for table accessfunction func.prop_lens(key)    return func.lens(        function(obj) return obj[key] end,        function(value)            return function(obj)                local result = func.merge(obj) -- Copy the object                result[key] = value                return result            end        end    )end---Curried lens get operation---@generic S, A---@param lens table<string, function> Lens object---@return fun(s: S): A Function that gets value using lensfunction func.view(lens)    return lens.getend---Curried lens set operation---@generic S, A---@param lens table<string, function> Lens object---@param value A Value to set---@return fun(s: S): S Function that sets value using lensfunction func.set(lens, value)    return lens.set(value)end---Curried lens modify operation---@generic S, A---@param lens table<string, function> Lens object---@param f fun(a: A): A Function to modify value---@return fun(s: S): S Function that modifies value using lensfunction func.over(lens, f)    return lens.modify(f)end---@alias Reducer<A,B> fun(acc: A, input: B): A---@alias Transducer<A,B,C> fun(reducer: Reducer<A,B>): Reducer<A,C>---Create a transducer for efficient composition of map/filter operations---@generic A, B, C---@param xform fun(reducer: Reducer<A,B>): Reducer<A,C> Transformation function---@return Transducer<A,B,C> Transducer functionfunction func.transducer(xform)    return function(reducer)        return xform(reducer)    endend---Map transducer---@generic T, U---@param f fun(x: T): U Mapping function---@return function Map transducerfunction func.map_t(f)    return func.transducer(function(reducer)        return function(acc, input)            return reducer(acc, f(input))        end    end)end---Filter transducer---@generic T---@param predicate fun(x: T): boolean Predicate function---@return function Filter transducerfunction func.filter_t(predicate)    return func.transducer(function(reducer)        return function(acc, input)            if predicate(input) then                return reducer(acc, input)            else                return acc            end        end    end)end---Compose transducers for efficient pipeline processing---@vararg function Transducers to compose---@return function Composed transducerfunction func.compose_t(...)    local transducers = { ... }    return function(reducer)        -- Apply transducers right to left (like function composition)        local composed_reducer = reducer        for i = #transducers, 1, -1 do            composed_reducer = transducers[i](composed_reducer)        end        return composed_reducer    endend-- ======================-- SAFE OPERATIONS & ERROR HANDLING - Defensive programming utilities-- ======================---Safe function call with error handling---@generic T---@param f function Function to call safely---@param ... any Arguments to pass---@return boolean, T Success flag and result/errorfunction func.safe_call(f, ...)    return pcall(f, ...)end---Maybe-style safe call---@generic T---@param f function Function to call---@param ... any Arguments---@return Maybe<T> Result wrapped in Maybefunction func.maybe_call(f, ...)    local success, result = pcall(f, ...)    if success then        return func.Maybe.just(result)    else        return func.Maybe.nothing    endend---Chain safe operations---@generic T---@param operations function[] Array of functions to chain---@return function Chained safe operationfunction func.chain_safe(operations)    checkType('Module:Functools.chain_safe', 1, operations, 'table')    return function(initial_value)        return func.foldl(function(acc)            return function(op)                return func.Maybe.bind(op)(acc)            end        end, func.Maybe.just(initial_value), unpack(operations))    endend-- ======================-- VALIDATION MODULE - Input validation and type checking-- ======================---@class Validationfunc.validation = {}---Validate a value against rules---@param value any Value to validate---@param rules table Validation rules---@return boolean|string True if valid, error message if invalidfunction func.validation.validate_value(value, rules)    if rules.required and (value == nil or value == "") then        return "Value is required"    end    if rules.type and type(value) ~= rules.type then        return string.format("Expected %s, got %s", rules.type, type(value))    end    if rules.min and type(value) == "number" and value < rules.min then        return string.format("Value must be at least %s", rules.min)    end    if rules.max and type(value) == "number" and value > rules.max then        return string.format("Value must be at most %s", rules.max)    end    return trueend---Validate options against rule set---@param options table Options to validate---@param rules table Rule set---@return string|nil Error message or nil if validfunction func.validation.validate_options(options, rules)    for key, rule in pairs(rules) do        local value = options[key]        local result = func.validation.validate_value(value, rule)        if result ~= true then            return string.format("%s: %s", key, result)        end    end    return nilend---Return default value if input is nil/empty---@generic T---@param value T|nil Input value---@param default T Default value---@return T Value or defaultfunction func.validation.default_value(value, default)    if value == nil or value == "" then        return default    end    return valueend-- ======================-- CURRIED CONVENIENCE FUNCTIONS - Easy-to-use curried versions-- ======================-- Curried versions with 'p' prefix (partial application)func.pmap = func.c2(func.map)func.pfilter = func.c2(func.filter)func.preduce = func.c3(func.reduce)-- Curried operationsfunc.add = func.c2(func.ops.add)func.mul = func.c2(func.ops.mul)func.sub = func.c2(func.ops.sub)func.div = func.c2(func.ops.div)func.mod = func.c2(func.ops.mod)func.pow = func.c2(func.ops.pow)func.eq = func.c2(func.ops.eq)func.lt = func.c2(func.ops.lt)func.gt = func.c2(func.ops.gt)-- Using thrush combinator for flip applicationfunc.tap = func.flip(func.apply)-- Using flip and composition for property accessfunc.prop = func.flip(func.comp(func.id))-- Using currying for simplified versionsfunc.add1 = func.add(1)func.sub1 = func.flip(func.sub)(1)func.double = func.mul(2)func.negate = func.mul(-1)func.not_empty = func.complement(func.is_empty)func.is_nil = function(x) return x == nil endfunc.not_nil = func.complement(func.is_nil)func.is_array = function(x) return type(x) == "table" and #x > 0 end-- Curried sequence operationsfunc.pzip = func.c2(func.zip)-- ======================-- MODULE EXPORT-- ======================return func
